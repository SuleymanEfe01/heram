...
exports.run = async(client, message, args) => {
    ...
    let sebep = args.slice(1).join(' ') || "Belirtilmemiş."
     ...
     if(user.id === message.author.id) return message.channel.send(' ``Kendini kicklayamazsın.``')
     if(user.id === client.user.id) return message.channel.send(' ``Botu kickleyemezsin.``')
  if(user.id === message.guild.ownerID) return message.channel.send(' ``Sunucu sahibini kickleyemezsin.``')
    if (!message.guild.member(user).kickable) return message.reply(' ``Bu kişinin rolü senden üstte veya `Üyeleri yasakla` yetkisine sahip.``');
