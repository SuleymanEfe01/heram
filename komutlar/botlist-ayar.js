...
**!botlist-ayar yetkili @rol**`)
  ...
  if(args[0] === "yetkili"){
   ...
   if(args[0] === "sıfırla"){
     if(!db.has(`bot-ekle_${message.guild.id}`)) return message.channel.send("Tüm herşeyi ayarlamazsan sıfırlayamam.")
     if(!db.has(`bot-log_${message.guild.id}`)) return message.channel.send("Tüm herşeyi ayarlamazsan sıfırlayamam.")
     if(!db.has(`basvuruk_${message.guild.id}`)) return message.channel.send("Tüm herşeyi ayarlamazsan sıfırlayamam.")
     if(!db.has(`basvuruk_${message.guild.id}`)) return message.channel.send("Tüm herşeyi ayarlamazsan sıfırlayamam.")
